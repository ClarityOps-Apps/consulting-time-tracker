...
final class AppDelegate: NSObject, NSApplicationDelegate, NSMenuDelegate {
    ...
    private func buildStatusItem() {
        ...
        item.button?.image = NSImage(systemSymbolName: "clock", accessibilityDescription: "Time")
    ...
    private func refreshStatusItem() {
        ...
        if button.image == nil {
            button.image = NSImage(systemSymbolName: "clock", accessibilityDescription: "Time")
            button.image?.isTemplate = true
            button.imagePosition = .imageLeft
        }
        let seconds = store.isRunning ? store.displaySeconds : 0
        button.title = " " + DurationFormat.menuBar(seconds)
    }
    ...
    private func makeWindow<V: View>(title: String, size: NSSize, root: V) -> NSWindow {
        ...
        window.backgroundColor = Theme.nsWindow
